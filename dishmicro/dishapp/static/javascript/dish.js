...
$("#submit_post").on("click",function (event) {
    let msg = ""
    if($("#post_title").val().length== 0&&$("#post_title").val().length<30&&$("#post_body").val()!=""&&$("#post_body").val().length<10000
    &&$("#post_pic").val() != ""){
        alert("Post Success")
        return
    }
    if($("#post_title").val().length==0||$("#post_title").val().length>30){
        msg += ("Please check your title \n")
    }
    if($("#post_body").val().length==0||$("#post_body").val().length>10000){
         msg +=("Please check your body \n")
    }
    if($("#post_pic").val().length == 0){
         msg +=("Please add an picture \n")
    }
